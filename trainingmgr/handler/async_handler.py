...
from trainingmgr.service.training_job_service import change_status_tj, get_data_extraction_in_progress_trainingjobs
...
def check_and_notify_feature_engineering_status(APP):
    ...
    while True:
        with LOCK:
            training_jobs = list(DATAEXTRACTION_JOBS_CACHE)

        for trainingjob in training_jobs:
            ...
            try:
                ...
                response = data_extraction_status(featuregroup_name, TRAININGMGR_CONFIG_OBJ)
                ...
                if response_data["task_status"] == "Completed":
                    with APP.app_context():
                        change_status_tj(trainingjob, Steps.DATA_EXTRACTION.name, States.FINISHED.name)
                        change_status_tj(trainingjob, Steps.DATA_EXTRACTION_AND_TRAINING.name, States.IN_PROGRESS.name)

                    kf_response = requests.post(
                        url_pipeline_run,
                        data=json.dumps({"trainingjob_name": featuregroup_name}),
                    ...
                    with LOCK:
                        DATAEXTRACTION_JOBS_CACHE.pop(trainingjob)
...
def start_async_handler(APP):
    """Start the asynchronous handler."""
    global DATAEXTRACTION_JOBS_CACHE
    ...
    threading.Thread(target=check_and_notify_feature_engineering_status, args=(APP,), daemon=True).start()
