...
def create_feature_group(
    ...
    try:
        ...
        ip = os.getenv("TRAINING_MANAGER_IP")
        tm_port = os.getenv("TRAINING_MANAGER_PORT")
        if not ip or not tm_port:
            raise TMException("TRAINING_MANAGER_IP/PORT not specified")

        url = f"http://{ip}:{tm_port}/ai-ml-model-training/v1/featureGroup"
