            ...
            if response.status_code == 200:
                ...
                for pipeline_info in pipelines_data.get('pipelines', []):
                    if pipeline_info['display_name'] == pipe_name:
                        pipe_dict = {
                            'arguments': {},  # Keeping this as is, since it's not clear where to get this from new structure
                            'description': pipeline_info.get('description'),
                            'id': pipeline_info['pipeline_id'],
                            'name': pipeline_info['display_name'],
                            'created_at': pipeline_info['created_at'],
                            'error': pipeline_info['error'],
                            'namespace': pipeline_info['namespace']
                        }
